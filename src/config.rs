...
impl Config {
    pub fn from(path: &Path) -> GdeResult<Self> {
        ...
        let v: Value = serde_json::from_str(&String::from_utf8_lossy(&std::fs::read(path)?))?;
    ...
    fn get_script(&self, script_type: &str, name: Option<&str>) -> GdeResult<Option<Vec<Value>>> {
        if let Some(value) = self.content.get(script_type) {
            let new_name: &str = if let Some(name) = name {
                name
            } else {
                let vec = value.as_object().unwrap().into_iter().collect::<Vec<_>>();

                // Empty
                if vec.is_empty() {
                    return Err(GdeError::ConfigError(format!(
                        "{} has empty content",
                        script_type
                    )));
                }
